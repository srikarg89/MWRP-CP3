...
int run_mtsp_2(int num_agents, int num_pivots, const std::vector<std::vector<int>>& cost_matrix, std::vector<int> initial_path, int upper_bound) {
    auto start = std::chrono::high_resolution_clock::now();
    IloEnv env;
    try {
        IloModel model(env);
        ...
        for(int agent = 0; agent < m; agent++) {
            ...
            // model.add(agent_cost <= L);
            model.add(agent_cost <= upper_bound);
            agent_cost.end();
        }

        // model.add(L <= upper_bound);
        // model.add(IloMinimize(env, L));
        model.add(IloMinimize(env, 0)); // Dummy objective since we're only checking feasibility.
...
int run_mtsp_binary_search(int num_agents, int num_pivots, const std::vector<std::vector<int>>& cost_matrix, const std::vector<int>& initial_path) {
    std::vector<int> greedy_solution = get_greedy_solution(cost_matrix, num_pivots);
    int upper_bound = 0;
    for(int i = 0; i < greedy_solution.size() - 1; i++) {
        int from = greedy_solution[i];
        int to = greedy_solution[i + 1];
        upper_bound += cost_matrix[from][to];
    }

    int lower_bound = 0;
    int best_solution = upper_bound;
    int guess = upper_bound;
    ...
    while(upper_bound > lower_bound + 1){
        ...
        printf("Trying upper bound: %d. Current bounds: [%d, %d]\n", guess, lower_bound, upper_bound);
        int result = run_mtsp_2(num_agents, num_pivots, cost_matrix, initial_path, guess);
        if(result != -1){
            // Feasible solution found with makespan <= mid.
            // best_solution = result;
            // upper_bound = result;
            best_solution = guess;
            upper_bound = guess;
        } else {
            // No feasible solution found with makespan <= mid.
            lower_bound = guess;
        }
    }
